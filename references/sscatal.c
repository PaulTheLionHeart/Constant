...
/* --------                                                            */
/*                                                                     */
/* This program calculates Catalan's constant G to many                */
/* decimal digits.                                                     */ 
/*                                                                     */
/*                                                                     */
/* Version 1.02:                                                       */
...
/*                                                                     */
/* 2004-02-04:                                                         */
/* Minor changes allowing larger computations with method 1.           */
/*                                                                     */
/* First version February 2004.                                        */
/*                                                                     */
/*                                                                     */
...
/* -----------                                                                                              */
/*                                                                                                          */
/* Catalan can be computed much faster using the following fast series:                                     */
/*                                                                                                          */
/*     1  inf (-1)^(n-1) 2^(8n) (40n^2-24n+3) [(2n)!]^3 (n!)^2                                              */
/* G = -- SUM ------------------------------------------------                                              */
/*     64 n=1             n^3 (2n-1) [(4n)!]^2                                                              */ 
/*                                                                                                          */
/* Each iteration of the series adds about 0.60 digits to Catalan.                                          */
/*                                                                                                          */
/*                                                                                                          */
/* Method = 2:                                                                                              */
/* -----------                                                                                              */
...
/*       inf    1           1            1             1             1             1            1           */
/* G = 3 SUM ------- (- --------- + ----------- - ----------- + ----------- - ----------- + ---------) -    */
/*       n=0 2^(4n)     2(8n+2)^2   2^2(8n+3)^2   2^3(8n+5)^2   2^3(8n+6)^2   2^4(8n+7)^2   2(8n+1)^2       */
/*                                                                                                          */
/*       inf    1           1            1             1             1              1              1        */
/*     2 SUM ------- (----------- + ----------- - ----------- - ------------ - ------------ + -----------)  */
/*       n=0 2^(12n)  2^4(8n+2)^2   2^6(8n+3)^2   2^9(8n+5)^2   2^10(8n+6)^2   2^12(8n+7)^2   2^3(8n+1)^2   */
/*                                                                                                          */
/*                                                                                                          */
/* Each iteration of the series adds about 1.20 digits to Catalan.                                          */
/*                                                                                                          */
/*                                                                     **************************************/
/* Accuracy:                                                           */
/* ---------                                                           */
...
/*                                                                     */
/* All printed digits are probably correct. The result for Catalan     */
...
/*                                                                     */
/*>gcc -O3 -o sscatal sscatal.c fftsg_h.c -lm                          */
/*                                                                     */
...
/* -------                                                             */
/*                                                                     */
/* Output is to the file "catalan.txt". This file name can easily be   */
/* changed.                                                            */
/*                                                                     */
/*                                                                     */
...
/* 256 kbyte at full CPU speed. The operating system was Fedora Core 1 */
/* Linux with the gcc 3.32 C-compiler. The CPU load was on average     */
...
/*---------------------------------------------------------------------*/
/* Catalan, method 1  188.43 s  2948.54 s   O(n*log(n)^3)  29.6 Mbyte  */
/* Catalan, method 2  241.99 s  3431.92 s   O(n*log(n)^3)  19.7 Mbyte  */
/*---------------------------------------------------------------------*/
/*                                                                     */
/***********************************************************************/
...
char version_string[80]="Program sscatal.c, version 1.02, 2007-08-13";

char file_name[20]="catalan.txt";    /* Output filename */
...
void sum_catalan_1(rP,rB,rQ,rT,n1,n2,maxsize)
bigint *rP;
bigint *rB;
bigint *rQ;
bigint *rT;
...
int maxsize;
{
   ...
   if ((n2-n1) == 1) {
      ...
      if (n == 0) {
         ...
         inttobig(&tmp,(40));
         inttobig(&tmp2,(n));
	 bigmul(&tmp3,&tmp,&tmp2);
	 bigmul(&tmp3,&tmp3,&tmp2);
	 inttobig(&tmp,(-24));
	 bigmul(&tmp4,&tmp2,&tmp);
	 inttobig(&tmp,(3));
	 bigadd(&tmp2,&tmp4,&tmp);
	 bigadd(rT,&tmp3,&tmp2);

         inttobig(&tmp2,(n));
	 inttobig(&tmp4,(256));
	 bigmul(&tmp4,&tmp4,&tmp2);
	 bigmul(&tmp4,&tmp4,&tmp2);

         inttobig(&tmp2,(2*n));
         inttobig(&tmp,(2*n-1));
	 bigmul(&tmp3,&tmp,&tmp2);
	 bigmul(&tmp2,&tmp3,&tmp3);
	 bigmul(&tmp2,&tmp2,&tmp3);
	 bigmul(rP,&tmp4,&tmp2);
	 ...
	 bigmul(rB,&tmp,&tmp2);
      ...
      sum_catalan_1(&LP,&LB,&LQ,&LT,n1,nm,maxsize);

      sum_catalan_1(&RP,&RB,&RQ,&RT,nm,n2,maxsize);
...
void sum_catalan_2(rP,rB,rQ,rT,n1,n2,maxsize,c1,c2,c3)
bigint *rP;
bigint *rB;
bigint *rQ;
bigint *rT;
...
int c3;
{
   ...
   if ((n2-n1) == 1) {
	 ...
	 bigmul(rB,&tmp,&tmp2);
      ...
      sum_catalan_2(&LP,&LB,&LQ,&LT,n1,nm,maxsize,c1,c2,c3);

      sum_catalan_2(&RP,&RB,&RQ,&RT,nm,n2,maxsize,c1,c2,c3);
...
void catalan_term_2(sum,c1,c2,c3,c4,sgn0,n1,n2,nn,bigsize)
short *sum;
int c1;
int c2;
int c3;
...
int bigsize;
{
   ...
   sum_catalan_2(&rP,&rB,&rQ,&rT,n1,n2,bigsize,c1,c2,c3);

   timediff1=clock()-time1;

   printf("Time term, fast series:         %10.2f\n",(double)(timediff1)/timetic);
...
char *argv[];
{
   ...
   if (argc != 3) {
      ...
      printf("%s\n",version_string);
      printf("\n");
      printf("method = 1 => Catalan's constant using fast series 1.\n");
      printf("method = 2 => Catalan's constant using fast series 2.\n");
      printf("\n");
      printf("Catalan is calculated with about 2^(power-of-2) decimal digits.\n");
      printf("\n");
...
if (method == 1) {   
   ...
   printf("\n");
   printf("Catalan's constant, method 1\n");
   printf("============================ \n");
   printf("\n");
   printf("%s\n",version_string);
   printf("\n");
   ...
   n1=0;
   n2=(int)((double)(decimals2)/0.60+0.5)+1;
   ...
   temp=(double)(bigsize);
   mem1=(int)(9.85*1048576.0*log(temp)*temp/(log(1048576.0/8.0)*1048576.0/8.0)+0.5);
   ...
   printf("Calculating Catalan ...\n");

   fftmaxerror=0.0;

   time1=clock();

   sum_catalan_1(&rP,&rB,&rQ,&rT,n1,n2,bigsize);

   timediff1=clock()-time1;

   printf("\n");

   printf("Time Catalan, fast series:      %10.2f\n",(double)(timediff1)/timetic);
   ...
   a = (short *)calloc(lsize+2,sizeof(short));
   b = (short *)calloc(lsize+2,sizeof(short));
   catalan_result = (short *)calloc(lsize+2,sizeof(short));
   ...
   printf("\n");
   printf("Time TOTAL Catalan:             %10.2f\n",(double)(timediff2)/timetic);
   ...
   lfprint(catalan_result,nn,outfile);  
...
if (method == 2) {   
   ...
   printf("\n");
   printf("Catalan's constant, method 2\n");
   printf("============================ \n");
   printf("\n");
   printf("%s\n",version_string);
   printf("\n");
   ...
   n1=0;
   n2=(int)((double)(decimals2)/1.20+0.5)+1;
   ...
   temp=(double)(bigsize);
   mem1=(int)(5.30*1048576.0*log(temp)*temp/(log(1048576.0/8.0)*1048576.0/8.0)+0.5);
...
/* Fast series sum 12 terms **************************************************/
   
   catalan_result = (short *)calloc(lsize+2,sizeof(short));

   lzero(catalan_result,nn);

   catalan_term_2(catalan_result,16,2,2,3,(-1),n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,16,4,3,3,1,n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,16,8,5,3,(-1),n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,16,8,6,3,1,n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,16,16,7,3,(-1),n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,16,2,1,3,1,n1,n2,nn,bigsize);

   catalan_term_2(catalan_result,4096,16,2,2,(-1),n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,4096,64,3,2,(-1),n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,4096,512,5,2,1,n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,4096,1024,6,2,1,n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,4096,4096,7,2,1,n1,n2,nn,bigsize);
   catalan_term_2(catalan_result,4096,8,1,2,(-1),n1,n2,nn,bigsize);
